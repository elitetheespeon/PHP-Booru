<?php
$logger = new logger();
$logger->log_action($f3->get('checked_user_id'), $_SERVER['REMOTE_ADDR'], 'POOL_VIEW', $f3->get('PARAMS.id'));
...
if($numrows == 0){
    ...
    if ($pg_start > $numrows || $pg_start < 0){
        $f3->reroute('/pool/view/'.$f3->get('PARAMS.id'));
	}
